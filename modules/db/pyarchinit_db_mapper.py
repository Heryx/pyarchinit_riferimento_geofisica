...
try:
	from pyarchinit_db_structure import Relashionship_check_table, Documentazione_table, PDF_administrator, US_table, UT_table, US_table_toimp, Site_table, Periodizzazione_table, Inventario_materiali_table, Struttura_table, Media_table, Media_thumb_table,Media_to_Entity_table, Tafonomia_table, Inventario_Lapidei_table, Inventario_materiali_table_toimp, Pyarchinit_thesaurus_sigle, SCHEDAIND_table, DETSESSO_table, DETETA_table, Archeozoology_table, Campioni_table, Geophysics_table
except:
	pass
...
try:
	...
	class GEOPHYSICS(object):
		...
		# def __repr__"
		def __repr__(self):
			return "<GEOPHYSICS('%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%r', '%r', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s', '%s', '%r', '%s', '%s')>" % (
