...
from pyarchinit_db_structure import Documentazione_table, PDF_administrator, US_table, UT_table, US_table_toimp, \
	...
	Media_to_Entity_table, Tafonomia_table, Inventario_materiali_table_toimp, Pyarchinit_thesaurus_sigle, SCHEDAIND_table, \
	DETSESSO_table, DETETA_table, Archeozoology_table, Campioni_table, Inventario_Lapidei_table, Geophysics_table

try:
	class US (object):
		# def __init__"
		def __init__(self, id_us, sito, area, us, d_stratigrafica, d_interpretativa, descrizione, interpretazione,
		             periodo_iniziale, fase_iniziale, periodo_finale, fase_finale, scavato, attivita, anno_scavo,
		             metodo_di_scavo, inclusi, campioni, rapporti, data_schedatura, schedatore, formazione,
		             stato_di_conservazione, colore, consistenza, struttura, cont_per, order_layer, documentazione,
		             unita_tipo,  # campi aggiunti per USM
		             settore, quad_par, ambient, saggio, elem_datanti, funz_statica, lavorazione, spess_giunti,
		             letti_posa, alt_mod, un_ed_riass, reimp, posa_opera, quota_min_usm, quota_max_usm, cons_legante,
		             col_legante, aggreg_legante, con_text_mat, col_materiale, inclusi_materiali_usm):
			self.id_us = id_us  # 0
			self.sito = sito  # 1
			self.area = area  # 2
			self.us = us  # 3
	...
	class UT (object):
		...
		def __repr__(self):
			return "<UT('%d', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%f', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')>" % (
				self.id_ut,  # 0        #sostituito %d con %s
				self.progetto,  # 1
				self.nr_ut,  # 2        #sostituito %d con %s
				self.ut_letterale,  # 3
				self.def_ut,  # 4
				...
				self.coord_piane,  # 18
				self.quota,  # 19       #sostituito %f con %s
	...
	class US_TOIMP (object):
		...
		def __repr__(self):
			return "<US_TOIMP('%d', '%s', '%s', '%d','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')>" % (
				self.id_us, self.sito, self.area, self.us, self.d_stratigrafica, self.d_interpretativa,
				self.descrizione, self.interpretazione, self.periodo_iniziale, self.fase_iniziale, self.periodo_finale,
				self.fase_finale, self.scavato, self.attivita, self.anno_scavo, self.metodo_di_scavo, self.inclusi,
				self.campioni, self.rapporti, self.data_schedatura, self.schedatore, self.formazione,
				self.stato_di_conservazione, self.colore, self.consistenza, self.struttura)
	...
	class SITE (object):
		# def __init__"
		def __init__(self, id_sito, sito, nazione, regione, comune, descrizione, provincia, definizione_sito,
		             find_check):
		...
		def __repr__(self):
			return "<SITE('%d','%s', '%s',%s,'%s','%s', '%s', '%s', '%d')>" % (
				self.id_sito, self.sito, self.nazione, self.regione, self.comune, self.descrizione, self.provincia,
				self.definizione_sito, self.find_check)
	...
	class PERIODIZZAZIONE (object):
		# def __init__"
		def __init__(self, id_perfas, sito, periodo, fase, cron_iniziale, cron_finale, descrizione, datazione_estesa,
		             cont_per):
		...
		def __repr__(self):
			return "<PERIODIZZAZIONE('%d', '%s', '%d', '%d', '%d', '%d', '%s', '%s', '%d')>" % (
				self.id_perfas, self.sito, self.periodo, self.fase, self.cron_iniziale, self.cron_finale,
				self.descrizione, self.datazione_estesa, self.cont_per)
	...
	class INVENTARIO_MATERIALI (object):
		# def __init__"
		def __init__(self, id_invmat, sito, numero_inventario, tipo_reperto, criterio_schedatura, definizione,
		             descrizione, area, us, lavato, nr_cassa, luogo_conservazione, stato_conservazione,
		             datazione_reperto, elementi_reperto, misurazioni, rif_biblio, tecnologie, forme_minime,
		             forme_massime, totale_frammenti, corpo_ceramico, rivestimento, diametro_orlo, peso, tipo, eve_orlo,
		...
		def __repr__(self):
			return "<INVENTARIO_MATERIALI('%d', '%s', '%d', '%s', '%s', '%s', '%s', '%d', '%d', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%d', '%s', '%s', '%r', '%r','%s', '%r', '%s', '%s' )>" % (
				self.id_invmat, self.sito, self.numero_inventario, self.tipo_reperto, self.criterio_schedatura,
				self.definizione, self.descrizione, self.area, self.us, self.lavato, self.nr_cassa,
				self.luogo_conservazione, self.stato_conservazione, self.datazione_reperto, self.elementi_reperto,
				self.misurazioni, self.rif_biblio, self.tecnologie, self.forme_minime, self.forme_massime,
				self.totale_frammenti, self.corpo_ceramico, self.rivestimento, self.diametro_orlo, self.peso, self.tipo,
				self.eve_orlo, self.repertato, self.diagnostico)
	...
	class INVENTARIO_MATERIALI_TOIMP (object):
		...
		def __init__(self, id_invmat, sito, numero_inventario, tipo_reperto, criterio_schedatura, definizione,
		             descrizione, area, us, lavato, nr_cassa, luogo_conservazione, stato_conservazione,
		             datazione_reperto, elementi_reperto, misurazioni, rif_biblio, tecnologie, forme_minime,
		             forme_massime, totale_frammenti, corpo_ceramico, rivestimento):
			self.id_invmat = id_invmat  # 0
			self.sito = sito  # 1
			self.numero_inventario = numero_inventario  # 2
			self.tipo_reperto = tipo_reperto  # 3
		...
		def __repr__(self):
			return "<INVENTARIO_MATERIALI_TOIMP('%d', '%s', '%d', '%s', '%s', '%s', '%s', '%d', '%d', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%d','%s', '%s')>" % (
				...
				self.luogo_conservazione, self.stato_conservazione, self.datazione_reperto, self.elementi_reperto,
				self.misurazioni, self.rif_biblio, self.tecnologie, self.forme_minime, self.forme_massime,
				self.totale_frammenti, self.corpo_ceramico, self.rivestimento)
	...
	class STRUTTURA (object):
		def __init__(self, id_struttura, sito, sigla_struttura, numero_struttura, categoria_struttura,
		             tipologia_struttura, definizione_struttura, descrizione, interpretazione, periodo_iniziale,
		             fase_iniziale, periodo_finale, fase_finale, datazione_estesa, materiali_impiegati,
		             elementi_strutturali, rapporti_struttura, misure_struttura):
		...
		def __repr__(self):
			return "<STRUTTURA('%d', '%s', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%d', '%d', '%s', '%s', '%s', '%s', '%s')>" % (
				self.id_struttura, self.sito, self.sigla_struttura, self.numero_struttura, self.categoria_struttura,
				self.tipologia_struttura, self.definizione_struttura, self.descrizione, self.interpretazione,
				self.periodo_iniziale, self.fase_iniziale, self.periodo_finale, self.fase_finale, self.datazione_estesa,
				self.materiali_impiegati, self.elementi_strutturali, self.rapporti_struttura, self.misure_struttura)
	...
	class MEDIA (object):
		# def __init__"
		def __init__(self, id_media, mediatype, filename, filetype, filepath, descrizione, tags):
	...
	class MEDIA_THUMB (object):
		# def __init__"
		def __init__(self, id_media_thumb, id_media, mediatype, media_filename, media_thumb_filename, filetype,
		             filepath):
		...
		def __repr__(self):
			return "<MEDIA('%d', '%d', '%s', '%s', %s, '%s', '%s')>" % (
				self.id_media_thumb, self.id_media, self.mediatype, self.media_filename, self.media_thumb_filename,
				self.filetype, self.filepath)
	...
	class MEDIATOENTITY (object):
		# def __init__"
		def __init__(self, id_mediaToEntity, id_entity, entity_type, table_name, id_media, filepath, media_name):
		...
		def __repr__(self):
			return "<MEDIATOENTITY('%d', '%d', '%s', '%s', '%d', '%s', '%s')>" % (
				self.id_mediaToEntity, self.id_entity, self.entity_type, self.table_name, self.id_media, self.filepath,
				self.media_name)
	...
	class TAFONOMIA (object):
		def __init__(self, id_tafonomia, sito, nr_scheda_taf, sigla_struttura, nr_struttura, nr_individuo, rito,
		             descrizione_taf, interpretazione_taf, segnacoli, canale_libatorio_si_no, oggetti_rinvenuti_esterno,
		             stato_di_conservazione, copertura_tipo, tipo_contenitore_resti, orientamento_asse,
		             orientamento_azimut, corredo_presenza, corredo_tipo, corredo_descrizione, lunghezza_scheletro,
		             posizione_scheletro, posizione_cranio, posizione_arti_superiori, posizione_arti_inferiori,
		             completo_si_no, disturbato_si_no, in_connessione_si_no, caratteristiche, periodo_iniziale,
		             fase_iniziale, periodo_finale, fase_finale, datazione_estesa, misure_tafonomia):
		...
		def __repr__(self):
			return "<TAFONOMIA('%d', '%s', '%d', '%s', '%d', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%r', '%s', '%s', '%s', '%r', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d','%d','%d','%d', '%s', '%s')>" % (
				self.id_tafonomia, self.sito, self.nr_scheda_taf, self.sigla_struttura, self.nr_struttura,
				self.nr_individuo, self.rito, self.descrizione_taf, self.interpretazione_taf, self.segnacoli,
				self.canale_libatorio_si_no, self.oggetti_rinvenuti_esterno, self.stato_di_conservazione,
				self.copertura_tipo, self.tipo_contenitore_resti, self.orientamento_asse, self.orientamento_azimut,
				self.corredo_presenza, self.corredo_tipo, self.corredo_descrizione, self.lunghezza_scheletro,
				self.posizione_scheletro, self.posizione_cranio, self.posizione_arti_superiori,
				self.posizione_arti_inferiori, self.completo_si_no, self.disturbato_si_no, self.in_connessione_si_no,
				self.caratteristiche, self.periodo_iniziale, self.fase_iniziale, self.periodo_finale, self.fase_finale,
				self.datazione_estesa, self.misure_tafonomia)
	...
	class PYARCHINIT_THESAURUS_SIGLE (object):
		# def __init__"
		def __init__(self, id_thesaurus_sigle, nome_tabella, sigla, sigla_estesa, descrizione, tipologia_sigla):
		...
		def __repr__(self):
			return "<PYARCHINIT_THESAURUS_SIGLE('%d', '%s', '%s', '%s', '%s', '%s')>" % (
				self.id_thesaurus_sigle, self.nome_tabella, self.sigla, self.sigla_estesa, self.descrizione,
	...
	class SCHEDAIND (object):
		# def __init__"
		def __init__(self, id_scheda_ind, sito, area, us, nr_individuo, data_schedatura, schedatore, sesso, eta_min,
		             eta_max, classi_eta, osservazioni):
		...
		def __repr__(self):
			return "<SCHEDAIND('%d','%s', '%d','%s','%d','%s','%s','%s','%d','%d','%s','%s')>" % (
				self.id_scheda_ind, self.sito, self.area, self.us, self.nr_individuo, self.data_schedatura,
				self.schedatore, self.sesso, self.eta_min, self.eta_max, self.classi_eta, self.osservazioni)
	...
	class DETSESSO (object):
		# def __init__"
		def __init__(self, id_det_sesso, sito, num_individuo, glab_grado_imp, pmast_grado_imp, pnuc_grado_imp,
		             pzig_grado_imp, arcsop_grado_imp, tub_grado_imp, pocc_grado_imp, inclfr_grado_imp, zig_grado_imp,
		             msorb_grado_imp, glab_valori, pmast_valori, pnuc_valori, pzig_valori, arcsop_valori, tub_valori,
		             pocc_valori, inclfr_valori, zig_valori, msorb_valori, palato_grado_imp, mfmand_grado_imp,
		             mento_grado_imp, anmand_grado_imp, minf_grado_imp, brmont_grado_imp, condm_grado_imp,
		             palato_valori, mfmand_valori, mento_valori, anmand_valori, minf_valori, brmont_valori,
		             condm_valori, sex_cr_tot, ind_cr_sex, sup_p_I, sup_p_II, sup_p_III, sup_p_sex, in_isch_I,
		             in_isch_II, in_isch_III, in_isch_sex, arco_c_sex, ramo_ip_I, ramo_ip_II, ramo_ip_III, ramo_ip_sex,
		             prop_ip_sex, ind_bac_sex):
		...
		def __repr__(self):
			return "<DETSESSO('%d','%s','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d', '%d', '%d', '%d', '%d', '%d','%d','%d','%d','%d','%r','%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')>" % (
				self.id_det_sesso, self.sito, self.num_individuo, self.glab_grado_imp, self.pmast_grado_imp,
				self.pnuc_grado_imp, self.pzig_grado_imp, self.arcsop_grado_imp, self.tub_grado_imp,
				self.pocc_grado_imp, self.inclfr_grado_imp, self.zig_grado_imp, self.msorb_grado_imp, self.glab_valori,
				self.pmast_valori, self.pnuc_valori, self.pzig_valori, self.arcsop_valori, self.tub_valori,
				self.pocc_valori, self.inclfr_valori, self.zig_valori, self.msorb_valori, self.palato_grado_imp,
				self.mfmand_grado_imp, self.mento_grado_imp, self.anmand_grado_imp, self.minf_grado_imp,
				self.brmont_grado_imp, self.condm_grado_imp, self.palato_valori, self.mfmand_valori, self.mento_valori,
				self.anmand_valori, self.minf_valori, self.brmont_valori, self.condm_valori, self.sex_cr_tot,
				self.ind_cr_sex, self.sup_p_I, self.sup_p_II, self.sup_p_III, self.sup_p_sex, self.in_isch_I,
				self.in_isch_II, self.in_isch_III, self.in_isch_sex, self.arco_c_sex, self.ramo_ip_I, self.ramo_ip_II,
				self.ramo_ip_III, self.ramo_ip_sex, self.prop_ip_sex, self.ind_bac_sex)
	...
	class DETETA (object):
		# def __init__"
		def __init__(self, id_det_eta, sito, nr_individuo, sinf_min, sinf_max, sinf_min_2, sinf_max_2, SSPIA, SSPIB,
		             SSPIC, SSPID, sup_aur_min, sup_aur_max, sup_aur_min_2, sup_aur_max_2, ms_sup_min, ms_sup_max,
		             ms_inf_min, ms_inf_max, usura_min, usura_max, Id_endo, Is_endo, IId_endo, IIs_endo, IIId_endo,
		             IIIs_endo, IV_endo, V_endo, VI_endo, VII_endo, VIIId_endo, VIIIs_endo, IXd_endo, IXs_endo, Xd_endo,
		             Xs_endo, endo_min, endo_max, volta_1, volta_2, volta_3, volta_4, volta_5, volta_6, volta_7, lat_6,
		             lat_7, lat_8, lat_9, lat_10, volta_min, volta_max, ant_lat_min, ant_lat_max, ecto_min, ecto_max):
		...
		def __repr__(self):
			return "<DETETA('%d','%s','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d')>" % (
				self.id_det_eta, self.sito, self.nr_individuo, self.sinf_min, self.sinf_max, self.sinf_min_2,
				self.sinf_max_2, self.SSPIA, self.SSPIB, self.SSPIC, self.SSPID, self.sup_aur_min, self.sup_aur_max,
				self.sup_aur_min_2, self.sup_aur_max_2, self.ms_sup_min, self.ms_sup_max, self.ms_inf_min,
				self.ms_inf_max,
				self.usura_min, self.usura_max, self.Id_endo, self.Is_endo, self.IId_endo, self.IIs_endo,
				self.IIId_endo,
				self.IIIs_endo, self.IV_endo, self.V_endo, self.VI_endo, self.VII_endo, self.VIIId_endo,
				self.VIIIs_endo,
				self.IXd_endo, self.IXs_endo, self.Xd_endo, self.Xs_endo, self.endo_min, self.endo_max, self.volta_1,
				self.volta_2, self.volta_3, self.volta_4, self.volta_5, self.volta_6, self.volta_7, self.lat_6,
				self.lat_7,
				self.lat_8, self.lat_9, self.lat_10, self.volta_min, self.volta_max, self.ant_lat_min, self.ant_lat_max,
				self.ecto_min, self.ecto_max)
	...
	class ARCHEOZOOLOGY (object):
		# def __init__"
		def __init__(self, id_archzoo, sito, area, us, quadrato, coord_x, coord_y, coord_z, bos_bison, calcinati,
		             camoscio, capriolo, cervo, combusto, coni, pdi, stambecco, strie, canidi, ursidi, megacero):
		...
		def __repr__(self):
			return "<ARCHEOZOOLOGY('%d', '%s', '%d', '%d', '%s', '%r', '%r', '%r', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d')>" % (
				self.id_archzoo, self.sito, self.area, self.us, self.quadrato, self.coord_x, self.coord_y, self.coord_z,
				self.bos_bison, self.calcinati, self.camoscio, self.capriolo, self.cervo, self.combusto, self.coni,
				self.pdi, self.stambecco, self.strie, self.canidi, self.ursidi, self.megacero)
	...
	class PDF_ADMINISTRATOR (object):
		# def __init__"
		def __init__(self, id_pdf_administrator, table_name, schema_griglia, schema_fusione_celle, modello):
		...
		# def __repr__"
		def __repr__(self):
			return "<PDF_ADMINISTRATOR('%d', '%s', '%s', '%s', '%s')>" % (self.id_pdf_administrator,  # 0
			                                                              self.table_name,  # 1
	...
	class CAMPIONI (object):
		# def __init__"
		def __init__(self, id_campione,  # 0
		             sito,  # 1
		             nr_campione,  # 2
		             tipo_campione,  # 3
		             descrizione,  # 4
		...
		# def __repr__"
		def __repr__(self):
			return "<CAMPIONI('%d', '%s', '%d', '%s', '%s', '%s', '%d', '%d', '%d', '%s')>" % (self.id_campione,  # 0
			                                                                                   self.sito,  # 1
			                                                                                   self.nr_campione,  # 2
			                                                                                   ...
			                                                                                   self.area,  # 5
			                                                                                   self.us,  # 6
			                                                                                   self.numero_inventario_materiale,
			                                                                                   # 7
			                                                                                   self.nr_cassa,  # 8
	...
	class DOCUMENTAZIONE (object):
		# def __init__"
		def __init__(self, id_documentazione,  # 0
		             sito,  # 1
		             nome_doc,  # 2
		             data,  # 3
	...
	class INVENTARIO_LAPIDEI (object):
		# def __init__"
		def __init__(self, id_invlap, sito, scheda_numero, collocazione, oggetto, tipologia, materiale, d_letto_posa,
		             d_letto_attesa, toro, spessore, larghezza, lunghezza, h, descrizione,
		             lavorazione_e_stato_di_conservazione, confronti, cronologia, bibliografia, compilatore):
		...
		def __repr__(self):
			return "<INVENTARIO_LAPIDEI('%d', '%s', '%d', '%s', '%s', '%s', '%s', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%s', '%s', '%s', '%s', '%s', '%s' )>" % (
				self.id_invlap, self.sito, self.scheda_numero, self.collocazione, self.oggetto, self.tipologia,
				self.materiale, self.d_letto_posa, self.d_letto_attesa, self.toro, self.spessore, self.larghezza,
				self.lunghezza, self.h, self.descrizione, self.lavorazione_e_stato_di_conservazione, self.confronti,
				self.cronologia, self.bibliografia, self.compilatore)
	...
	class GEOFISICA (object):
		# def __init__"
		def __init__(self, id_grid,         #0
				sito,                       #1
				progetto,                   #2
				metodo,                     #3
				anno,                       #4
			...
			self.y = y,                     # 17
			self.date = date,               # 18
			self.frequenza = frequenza,     # 19
			self.risoluzione = risoluzione, # 20
			self.max_prof = max_prof,       # 21
			self.range = range,             # 22
			self.bibliografia = bibliografia  #23
		
		def __repr__(self):
			return "<GEOFISICA('%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%r', '%r', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s', '%s', '%r', '%s', '%s', '%s')>" % (
				self.id_grid,           # 0
				self.sito,              # 1
				self.progetto,          # 2
				self.metodo,            # 3
				...
				self.max_prof,          # 21
				self.range,             # 22
				self.bibliografia)      # 23
	
	
	mapper (GEOFISICA, Geophysics_table.geophysics_table)
