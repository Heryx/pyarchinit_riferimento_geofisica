...
##			self.loadMediaPreview(1)
	...
	def generate_list_pdf(self):
		...
		for i in range(len(self.DATA_LIST)):
			...
			unicode(self.DATA_LIST[i].sito),								#1- contesto_provenienza
			int(self.DATA_LIST[i].scheda_numero),				#2- scheda_numero
			unicode(self.DATA_LIST[i].collocazione),					#3 - collocazione
			unicode(self.DATA_LIST[i].oggetto),			#4 - oggetto
			unicode(self.DATA_LIST[i].tipologia),					#5 - tipologia
			unicode(self.DATA_LIST[i].materiale),					#6 - materiale
			unicode(self.DATA_LIST[i].d_letto_posa),								#7 - D_letto_posa
			unicode(self.DATA_LIST[i].d_letto_attesa),							#8 - d_letto_attesa
			unicode(self.DATA_LIST[i].toro), 						#9 - toro
			unicode(self.DATA_LIST[i].spessore),		#10 - spessore
			unicode(self.DATA_LIST[i].larghezza),		#11 - larghezza
			unicode(self.DATA_LIST[i].lunghezza),			#12 - lunghezza
			unicode(self.DATA_LIST[i].h),			#13 - h
			unicode(self.DATA_LIST[i].descrizione),					#14 - descrizione
			unicode(self.DATA_LIST[i].lavorazione_e_stato_di_conservazione),						#15 - lavorazione_e_stato_di_conservazione
			unicode(self.DATA_LIST[i].confronti),						#16 - confronti
			unicode(self.DATA_LIST[i].cronologia),								#17 - .cronologia
			unicode(self.DATA_LIST[i].bibliografia),				#18 - .bibliografia
			unicode(self.DATA_LIST[i].compilatore)				#19 - autore scheda
		])
		return data_list
	...
	def insert_new_rec(self):
		...
		try:
			...
			self.DB_MANAGER.max_num_id(self.MAPPER_TABLE_CLASS, self.ID_TABLE)+1, 			#0 - IDsito
						...
						unicode(self.lineEdit_collocazione.text()), 											#3 - tipo_reperto
						unicode(self.comboBox_oggetto.currentText()),									#4 - criterio
						unicode(self.comboBox_tipologia.currentText()), 											#5 - definizione
						unicode(self.comboBox_materiale.currentText()),								#6 - descrizione
						...
						lunghezza,
						h,
						unicode(self.textEdit_descrizione.toPlainText()),
						unicode(self.textEdit_lavorazione_e_stato_di_conservazione.toPlainText()),												#11 - luogo conservazione
						unicode(self.textEdit_confronti.toPlainText()),
						unicode(self.lineEdit_cronologia.text()), 											#5 - definizione
						str(bibliografia),
						unicode(self.lineEdit_compilatore.text()),										#16 - rif biblio
						)
...
##						self.loadMediaPreview(0)
	...
	def on_pushButton_search_go_pressed(self):
		...
		if self.BROWSE_STATUS != "f":
			...
			self.TABLE_FIELDS[2] : "'" + unicode(self.lineEdit_collocazione.text()) + "'",
			self.TABLE_FIELDS[3] : "'" + unicode(self.comboBox_oggetto.currentText()) + "'",
			self.TABLE_FIELDS[4] : "'" + unicode(self.comboBox_tipologia.currentText()) + "'",
			self.TABLE_FIELDS[5] : "'" + unicode(self.textEdit_materiale.text()) + "'",
			...
			self.TABLE_FIELDS[18] : "'" + unicode(self.lineEdit_compilatore.text()) + "'",
			}
...
##			self.DATA_LIST.append(i)
	...
	def fill_fields(self, n=0):
		...
		try:
			...
			unicode(self.comboBox_tipologia.setEditText(self.DATA_LIST[self.rec_num].tipologia))				#3 - tipologia
			unicode(self.comboBox_materiale.setEditText(self.DATA_LIST[self.rec_num].materiale))				#4 - materiale
			unicode(self.comboBox_oggetto.setEditText(self.DATA_LIST[self.rec_num].oggetto))					#5 - oggetto
			unicode(self.textEdit_descrizione.setText(self.DATA_LIST[self.rec_num].descrizione))				#6 - descrizione
			unicode(self.textEdit_lavorazione_e_stato_di_conservazione.setText(self.DATA_LIST[self.rec_num].lavorazione_e_stato_di_conservazione))				#6 - descrizione
			...
			unicode(self.textEdit_confronti.setText(self.DATA_LIST[self.rec_num].confronti))				#6 - descrizione
...
##########
	...
	def set_LIST_REC_TEMP(self):
		...
		unicode(self.lineEdit_collocazione.text()), 								#3 - collocazione
		unicode(self.comboBox_oggetto.currentText()),								#4 - oggetto
		unicode(self.comboBox_tipologia.currentText()), 							#5 - tipologia
		unicode(self.comboBox_materiale.currentText()), 							#6 - materiale
		unicode(d_letto_posa),														#8 - d_letto_posa
		unicode(d_letto_attesa),													#9 - d_letto_attesa
		unicode(toro),																#10 - toro
		unicode(spessore),															#11 - spessore
		unicode(larghezza),															#12 - larghezza
		unicode(lunghezza),															#13 - lunghezza
		unicode(h),																	#14 - h
		unicode(self.textEdit_descrizione.toPlainText()),							#15 - descrizione
		unicode(self.textEdit_lavorazione_e_stato_di_conservazione.toPlainText()),	#16 - lavorazione
		unicode(self.textEdit_confronti.toPlainText()),							#17 - confronti
		unicode(self.lineEdit_cronologia.text()),									#18 - cronologia
		unicode(bibliografia),														#19 - bibliografia
		unicode(self.lineEdit_compilatore.text()),									#20 - compilatore
		]
