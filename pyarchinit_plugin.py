...
class PyArchInitPlugin:
	...
	def initGui(self):
		...
		self.actionGeo = QAction(QIcon(icon_Geo), "Geophysics", self.iface.mainWindow())
		self.actionGeo.setWhatsThis("Geophysics")
		QObject.connect(self.actionGeo, SIGNAL("triggered()"), self.runGeo)

		self.dataToolButton.addActions( [ self.actionSite, self.actionUS, self.actionInr, self.actionCampioni, self.actionLapidei, self.actionGeo ] )
...
##			self.actionInfo.setCheckable(True)
		...
		self.iface.addPluginToMenu("&pyArchInit - Archaeological GIS Tools", self.actionGeo)
		...
		self.menu.addActions([self.actionSite, self.actionUS, self.actionInr, self.actionCampioni, self.actionLapidei, self.actionGeo])
...
##
	...
	def runGeo(self):
		pluginGui = pyarchinit_Geophysics(self.iface)
		pluginGui.show()
		self.pluginGui = pluginGui  # save
	...
	def unload(self):
		...
		self.iface.removePluginMenu("&pyArchInit - Archaeological GIS Tools",self.actionGeo)
		if self.PARAMS_DICT['EXPERIMENTAL'] == 'Si':
		...
		self.iface.removeToolBarIcon(self.actionGeo)
		if self.PARAMS_DICT['EXPERIMENTAL'] == 'Si':
